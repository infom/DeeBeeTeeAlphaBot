...
namespace Telegram
{
    ...
    class Method
    {
        ...
        public string GetMe()
        {
            using (WebClient webClient = new WebClient())
            {
                string response = webClient.DownloadString(_link + _token + "/getme");
                return response;
            }
        }
        ...
        public void SendMessage(string message, int ChatId)
        {
            using (WebClient webClient = new WebClient())
            {
                ...
                webClient.UploadValues(_link + _token + "/sendMessage", pars);
             }                
        }
        public void ForwardMessage(int fromChatID, int chatId, int messageID)
        {
            using (WebClient webClient = new WebClient())
            {
                ...
                webClient.UploadValues(_link + _token + "/forwardMessage", pars);
            }
        }
        async public Task SendPhotoInputFile(int ChatID, string pathToPhoto, string caption = "")
        {
            ...
            {
                string url = _link + _token + "/sendPhoto";
        ...
        public void SendPhoto_link(int ChatID, string _linkToPhoto, string caption = "")
        {
            using (WebClient webClient = new WebClient())
            {
                NameValueCollection pars = new NameValueCollection();
                pars.Add("chat_id", ChatID.ToString());
                pars.Add("photo", _linkToPhoto);
                pars.Add("caption", caption);
                webClient.UploadValues(_link + _token + "/sendPhoto", pars);
            }
        }
        async public Task SendAudioInputFile(int ChatID, string pathToAudio, string caption = "", int duration = 0, string performer = "", string title = "")
        {
            using (MultipartFormDataContent form = new MultipartFormDataContent())
            {
                string url = _link + _token + "/sendAudio";
        ...
        public void SendAudio_link(int ChatID, string _linkToAudio, string caption = "")
        {
            using (WebClient webClient = new WebClient())
            {
                NameValueCollection pars = new NameValueCollection();
                pars.Add("chat_id", ChatID.ToString());
                pars.Add("audio", _linkToAudio);
                pars.Add("caption", caption);
                webClient.UploadValues(_link + _token + "/sendAudio", pars);
            }
        }
        async public Task SendDocumentInputFile(int ChatID, string pathToDocument, string caption = "")
        {
            using (MultipartFormDataContent form = new MultipartFormDataContent())
            {
                string url = _link + _token + "/sendDocument";
        ...
        public void SendDocumento_link(int ChatID, string _linkToDocument, string caption = "")
        {
            using (WebClient webClient = new WebClient())
            {
                NameValueCollection pars = new NameValueCollection();
                pars.Add("chat_id", ChatID.ToString());
                pars.Add("document", _linkToDocument);
                pars.Add("caption", caption);
                webClient.UploadValues(_link + _token + "/sendDocument", pars);
            }
        }
        ...
        public void SendVideo_link(int chatID, string _linkToVideo, string caption = "")
        {
            using (WebClient webClient = new WebClient())
            {
                NameValueCollection pars = new NameValueCollection();
        ...
        public void SendVoice_link(int chatID, string _linkToAudio, string caption = "")
        {
            using (WebClient webClient = new WebClient())
            {
                NameValueCollection pars = new NameValueCollection();
        ...
        public string getUserProfilePhotos(int user_id, int offset, int limit = 100)
        {
            using (WebClient webClient = new WebClient())
            {
                string response = webClient.DownloadString(_link + _token + "/getUserProfilePhotos?user_id=" + user_id + "&limit=" + limit + "&offset=" + offset);
                JSONNode N = JSON.Parse(response);
                N = N["result"]["photos"].AsArray[0];
                string _linkPhoto = N[N.Count - 1]["file_id"];
                return _linkPhoto;
            }
        }
        ...
        public string getFile(string file_id)
        {
            using (WebClient webClient = new WebClient())
            {
                string response = webClient.DownloadString(_link + _token + "/getFile?file_id=" + file_id);
                JSONNode N = JSON.Parse(response);
